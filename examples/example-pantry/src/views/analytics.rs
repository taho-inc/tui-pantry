...
fn render_bandwidth_gauges(area: Rect, buf: &mut Buffer) {
    let metrics: &[(&str, f64)] = &[("Inbound", 0.42), ("Outbound", 0.67), ("Cache Hit", 0.91)];
...
fn render_latency_chart(area: Rect, buf: &mut Buffer) {
    ...
    let y_labels =
        ["0ms", "50ms", "100ms"].map(|s| Span::styled(s, Style::default().fg(MOCHA.text_dim)));
...
fn render_error_scatter(area: Rect, buf: &mut Buffer) {
    ...
    let x_labels =
        ["0m", "6m", "12m"].map(|s| Span::styled(s, Style::default().fg(MOCHA.text_dim)));
    let y_labels = ["0", "8", "16"].map(|s| Span::styled(s, Style::default().fg(MOCHA.text_dim)));
    ...
    Chart::new(datasets)
        ...
        .y_axis(Axis::default().bounds([0.0, 16.0]).labels(y_labels))
        .render(area, buf);
}
...
impl Ingredient for AnalyticsDefault {
    ...
    fn source(&self) -> &str {
        "example_pantry::views::analytics"
    }

    fn render(&self, area: Rect, buf: &mut Buffer) {
        let [gauge_row, chart_row] =
            Layout::vertical([Constraint::Length(3), Constraint::Min(10)]).areas(area);
